...
class UploadJobViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def add_file(self, request, pk=None):
        ...
        # add it to the list of files in the db
    ...
    def process(self, request, pk=None):
        """
        Calls a celery task to create an SIP on the specified UploadJob and then archive it.
        Saves SIP in BIC_UPLOAD_PATH (current working directory if not declared).
        In case of direct SIP upload, the celery task simply moves the package.
        """
        uj = self.get_object()

        # Create an empty Archive and Step, will get populated by the celery task
        archive = Archive.objects.create(
            recid="", source="local", source_url="", creator=request.user
        )
        step = Step.objects.create(
