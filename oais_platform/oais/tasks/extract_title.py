...
from oais_platform.oais.tasks.utils import extract_sip_zip
...
def extract_title(self, archive_id, step_id):
    ...
    try:
        ...
        logger.warning(
            f"Error while extracting title from dc.xml in {sip_zip_path}: {str(e)}"
        )
        return {
            "status": 1,
            "errormsg": f"Title could not be extracted from Dublin Core file in {sip_zip_path}",
        }
