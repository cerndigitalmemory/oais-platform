...
def validate(self, archive_id, step_id):
    ...
    sip_path = archive.path_to_sip

    logger.info(f"Starting SIP validation {sip_path}")

    step = Step.objects.get(pk=step_id)
    step.set_status(Status.IN_PROGRESS)

    sip_exists = os.path.exists(sip_path)

    if not sip_exists:
        step.set_failure_type(StepFailureType.PATH_NOT_FOUND)
        return {"status": 1, "errormsg": "SIP does not exist"}

    with sip_as_directory(sip_path) as sip_folder_name:
