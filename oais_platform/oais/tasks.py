...
def finalize(self, status, retval, task_id, args, kwargs, einfo):
    ...
    if status == states.SUCCESS:
        ...
        if retval["status"] == 0:
            ...
            # If harvest is completed then add the audit of the sip.json to the archive.manifest field
            if step.name == 2:
                ...
                try:
                    with open(sip_location) as json_file:
                        ...
                        #sip_json_string = json.dumps(json_audit)
                        print(json_audit, type(json_audit))
                        archive.set_archive_manifest(json_audit)
                        logging.info(f"Sip.json saved at manifest field")
                except:
