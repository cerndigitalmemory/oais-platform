...
def _set_and_return_error(step, errormsg, extra_log=None, log_warning=False):
    """
    ...
    if log_warning:
        logger.warning(errormsg + (f" {extra_log}" if extra_log else ""))
    else:
...
def _remove_periodic_task_on_failure(task_name, step, output_data):
    ...
    """
    _set_and_return_error(
        step, f"Step {step.id} failed. Step status: {step.status}", log_warning=True
    )
